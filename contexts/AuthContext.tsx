...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signOut = async () => {
    try {
      ...
      localStorage.removeItem('sb-yutarvvbovvomsbtegrk-auth-token');
  ...
  const resetPassword = async (email: string) => {
    try {
      ...
      if (typeof window === 'undefined') {
        return { error: new Error('Password reset must be triggered from the client side') };
      }
      ...
      // Use the correct Supabase auth method for password reset
